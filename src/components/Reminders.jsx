...
const Reminders = () => {
  const [reminders, setReminders] = useState(() => {
    // Load reminders from cookies when the component mounts
    const savedReminders = JSON.parse(document.cookie.split("; ").find(row => row.startsWith("reminders="))?.split("=")[1] || "[]");
  ...
  useEffect(() => {
    document.cookie = `reminders=${JSON.stringify(reminders)}; path=/; max-age=86400`; // 1 day expiration
  }, [reminders]);
